...
def generator_agent(analysis_result, sample_data, openai_api_key, model, num_rows_to_generate):
    openai.api_key = openai_api_key
    user_message = f"Generate {num_rows_to_generate} rows of data based on the following analysis:\n{analysis_result}\nSample data:\n{sample_data}" if num_rows_to_generate else f"Generate data based on the following analysis:\n{analysis_result}\nSample data:\n{sample_data}"
    response = openai.ChatCompletion.create(
        model=model,
        messages=[
            {
...
def main():
    ...
    num_rows_to_generate = st.number_input("Enter the number of rows to generate (leave empty to generate all rows):", min_value=0, value=6, step=1)
